...
function lookupLink(node, keys) {
  ...
  if (!(node instanceof document.NavLink)) {
    throw new errors.LinkLookupError('Invalid link lookup: ' + JSON.stringify(keys));
  }
  return node;
}
...
var Client = function () {
  ...
  _createClass(Client, [{
    ...
    value: function get(url) {
      var link = new document.NavLink(url, 'get');
...
function primitiveToNode(data, baseUrl) {
  ...
  if (isObject && data._type === 'document') {
    ...
    return new document.NavLink(_url, method, 'application/json', fields, _title, _description);
  } else if (isObject) {
    // Object
    var _content = {};
    for (var key in data) {
...
var NavLink = function NavLink(url, method) {
